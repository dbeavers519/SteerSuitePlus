...
bool SimulationEngine::_simulateOneStep()
{
	...
	for ( agentIterator = _agents.begin(); agentIterator != _agents.end(); ++agentIterator )
	{
		if ((*agentIterator)->enabled()){
			...
			// Output agent position at each frame
			//std::cout << "Agent " << count << " Position: " << (*agentIterator)->position() << std::endl;
		}
		else {
...
void SimulationEngine::addWaitingAgent(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::ModuleInterface * owner)
{
	// Adds a new agent to the waiting list
	// Could be done more efficiently (queue based on start time), but this works for now
	_waitList.push_back(std::pair<SteerLib::AgentInitialConditions, SteerLib::ModuleInterface *>(initialConditions, owner));
}
